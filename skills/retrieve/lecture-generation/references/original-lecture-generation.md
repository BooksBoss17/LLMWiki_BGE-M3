...
原创或改编题图交给 `physics-diagram-toolkit`：题干中的图使用 `purpose=question` 和黑白 `exam-monochrome`，不得含推导受力；答案解析新增的分析图使用 `purpose=solution` 和 `solution-color`。把两类 asset、completion 和 SHA-256 登记到 `diagram_assets.json`，在导出前运行 `scripts/validate_diagram_asset_separation.py`，禁止解析图进入练习文件。
...
5. **习题集/课堂练习** — 不套框，保留图像题；答案、解析和提示进入独立答案文件

**内部溯源规范：** 将教材、题库和讲义来源写入任务内 `question_source_map.json`；对外练习与答案只显示本套材料的本地题号，不显示题库 ID、原题号、源路径或内部来源标签。
...
- [ ] 例题含完整题干+答案+详解+方法点拨
- [ ] 公式用 LaTeX 闭合（$...$ 或 $$...$$）
- [ ] 内部题源映射完整，且对外文件未显示题库 ID、原题号、源路径或内部来源标签
- [ ] 文件保存到 output/ 目录
- [ ] 知识框架用表格而非 ASCII 树状图
